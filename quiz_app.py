...
def save_results(results):
    ...
    if not os.path.exists(results_file):
        headers = ['date', 'start_time', 'end_time', 'total_time_spent', 'email_id', 
                   'no_qns_attempted', 'qns_answered_correctly', 'score']
        pd.DataFrame(columns=headers).to_excel(results_file, index=False, sheet_name='Sheet1')
    
    existing_data = pd.read_excel(results_file, sheet_name='Sheet1')
...
def main():
    ...
    if st.session_state.stage == 'email':
        st.session_state.email = st.text_input("Enter your email ID:")
